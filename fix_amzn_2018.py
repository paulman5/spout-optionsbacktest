#!/usr/bin/env python3
"""
Fix AMZN 2018 data to include all historical columns like other tickers.
"""
...
def extract_strike_from_ticker(ticker):
    ...
    Example: O:AMZN180216C00850000 -> 85000000
    """
...
def fix_amzn_2018_data(file_path):
    """Fix AMZN 2018 data to include all historical columns."""
...
def main():
    ...
    # Process monthly and weekly 2018 files
    files_to_process = [
        os.path.join(amzn_dir, "monthly", "2018_options_pessimistic.csv"),
        os.path.join(amzn_dir, "weekly", "2018_options_pessimistic.csv")
    ]
    
    for file_path in files_to_process:
        if os.path.exists(file_path):
            fix_amzn_2018_data(file_path)
        else:
            print(f"⚠️ File not found: {file_path}")
    
    print("\n✅ AMZN 2018 data fixed to include all historical columns!")
