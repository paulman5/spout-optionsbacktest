#!/usr/bin/env python3
"""
Revert XLK data and apply 2-for-1 split only before December 5, 2025
"""

import pandas as pd
from pathlib import Path

def revert_and_apply_split(file_path, split_date, split_ratio):
...
def main():
    """Main function to revert and apply XLK split"""
    print("🔄 Reverting XLK data and applying 2-for-1 split only before December 5, 2025...")
    
    split_date = pd.to_datetime('2025-12-05')
    split_ratio = 2
    
    base_dir = Path("/Users/paulvanmierlo/spout-optionsbacktest/python-boilerplate/data/XLK")
    years = [2025, 2024]  # Fix 2025 and 2024 data
    ...
    for year in years:
        ...
        for freq in frequencies:
            ...
            if file_path.exists():
                rows_processed = revert_and_apply_split(file_path, split_date, split_ratio)
                total_rows_processed += rows_processed
            else:
                print(f"   File not found: {file_path}")
    
    print(f"\n✅ XLK data reverted and split applied!")
