#!/usr/bin/env python3
"""
Revert XLK strike prices to original state (multiply by 2)
"""

import pandas as pd
from pathlib import Path

def revert_xlk_strikes(file_path):
    """Revert XLK strike prices to original state"""
    ...
    # Revert all strike prices by multiplying by 2
    df['strike'] = df['strike'] * 2
    ...
    print(f"   Reverted strike prices (multiplied by 2)")
    ...
    print(f"   ✅ Reverted file saved: {len(df)} total rows")
    
    return len(df)

def main():
    """Main function to revert XLK strike prices"""
    print("🔄 Reverting XLK strike prices to original state...")
    
    base_dir = Path("/Users/paulvanmierlo/spout-optionsbacktest/python-boilerplate/data/XLK")
    years = [2021, 2022, 2023, 2024, 2025]  # XLK years that exist
    ...
    for year in years:
        ...
        for freq in frequencies:
            ...
            if file_path.exists():
                rows_processed = revert_xlk_strikes(file_path)
                total_rows_processed += rows_processed
            else:
                print(f"   File not found: {file_path}")
    
    print(f"\n✅ XLK strike prices reverted!")
