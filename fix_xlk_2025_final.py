#!/usr/bin/env python3
"""
Fix XLK 2025 data - ensure original strikes are preserved after December 5, 2025
"""

import pandas as pd
from pathlib import Path

def fix_xlk_2025_data(file_path, split_date, split_ratio):
    """Fix 2025 data - ensure original strikes are preserved after split date"""
    print(f"Fixing 2025 data {file_path}...")
    ...
    if len(after_split) > 0:
        ...
        # Strikes are already extracted from tickers, so they should be original
...
def main():
    """Main function to fix XLK 2025 data"""
    print("🔄 Fixing XLK 2025 data - preserve original strikes after December 5, 2025...")
    
    split_date = pd.to_datetime('2025-12-05')
    split_ratio = 2
    
    base_dir = Path("/Users/paulvanmierlo/spout-optionsbacktest/python-boilerplate/data/XLK")
    years = [2025]  # Fix 2025 data only
    ...
    for year in years:
        ...
        for freq in frequencies:
            ...
            if file_path.exists():
                rows_processed = fix_xlk_2025_data(file_path, split_date, split_ratio)
