#!/usr/bin/env python3
"""
Revert 40-for-1 stock split to NVDA data before July 20, 2021
"""
...
def revert_stock_split_to_file(file_path):
    """Revert 40-for-1 stock split to a single file"""
    ...
    # Revert 40-for-1 split to data before July 20, 2021
    # Multiply strike and all price columns by 40
    
    # Columns to multiply by 40
    price_columns = [
    ...
    print(f"   ✅ Reverted 40-for-1 stock split for {before_split_count} rows before July 20, 2021")
    
    return before_split_count

def main():
    """Main function to process all NVDA files"""
    print("🔄 Reverting 40-for-1 stock split to NVDA data before July 20, 2021...")
    ...
    for year in years:
        ...
        for freq in frequencies:
            ...
            if file_path.exists():
                rows_processed = revert_stock_split_to_file(file_path)
                total_rows_processed += rows_processed
            else:
                print(f"   File not found: {file_path}")
    
    print(f"\n✅ 40-for-1 stock split reverted for NVDA data!")
