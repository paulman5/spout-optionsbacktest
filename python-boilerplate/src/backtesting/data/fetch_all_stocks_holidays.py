#!/usr/bin/env python3
"""
Fetch weekly options with holiday adjustments for ALL stocks:
...
- Use greeks2.py for IV and probability ITM
...
# Stocks used for graphs (process first)
PRIORITY_STOCKS = [
...
# Market holidays by year (same as AAPL)
MARKET_HOLIDAYS = {
    2016: [
...
def parse_option_ticker(ticker: str, entry_date: date = None):
    """
    Parse option ticker format: O:AAPL180126C00197500
...
print("=" * 80)
print("FETCHING ALL STOCKS FOR HOLIDAYS FOLDER")
...
print("\n📊 Discovering stocks...", flush=True)
data_dir = base_path / "data"
all_stock_dirs = [d.name for d in data_dir.iterdir() if d.is_dir() and not d.name.startswith('.')]
all_stock_dirs.sort()

# Separate priority stocks and others
priority_stocks = [s for s in PRIORITY_STOCKS if s in all_stock_dirs]
other_stocks = [s for s in all_stock_dirs if s not in PRIORITY_STOCKS]
all_stocks = priority_stocks + other_stocks

print(f"   Found {len(all_stocks)} stocks total")
print(f"   Priority stocks (for graphs): {len(priority_stocks)}")
print(f"   Other stocks: {len(other_stocks)}")
print(f"   Priority: {', '.join(priority_stocks)}")
...
try:
    ...
    LIMIT 1;
    """).fetchone()
    print(f"✅ S3 connection test successful (found files in test query)", flush=True)
except Exception as e:
    print(f"❌ S3 connection test FAILED: {e}", flush=True)
    print(f"   Cannot proceed without S3 access. Please check your credentials and network.", flush=True)
    print(f"   Endpoint: {endpoint_clean}, Bucket: {S3_BUCKET}, Region: {S3_REGION}", flush=True)
    sys.exit(1)

# Start with AMZN (Amazon) as requested
# Reorder stocks to start with AMZN
if 'AMZN' in all_stocks:
    all_stocks.remove('AMZN')
    all_stocks = ['AMZN'] + all_stocks
    print(f"\n📌 Starting with AMZN as requested")
    print(f"   Stock order: {', '.join(all_stocks[:10])}...")
...
for TICKER in all_stocks:
    ...
    for TEST_YEAR in YEARS:
        ...
        if not table_exists:
            ...
            # Retry logic for S3 connection - but fail if it doesn't work
            max_retries = 5
            retry_delay = 10
            all_files = None
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        print(f"   ❌ STOPPING - S3 connection must work to proceed", flush=True)
                        raise RuntimeError(f"S3 connection failed for {TICKER} {TEST_YEAR} after {max_retries} attempts")
            
            if not all_files:
                print(f"   ❌ No files found for {TEST_YEAR} - this should not happen if S3 is working", flush=True)
        ...
        if not stock_file.exists():
            ...
            def calc_iv_prob(row):
                try:
                    # Use mid_price instead of close_price
...
print("\n" + "=" * 80)
print("✅ ALL STOCKS AND YEARS COMPLETE!")
print("=" * 80)
print(f"Processed {len(all_stocks)} stocks across {len(YEARS)} years")
