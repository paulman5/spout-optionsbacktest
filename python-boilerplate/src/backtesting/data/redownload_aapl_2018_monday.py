...
- Monday entries only (days_to_expiry=5, Monday to Friday)
- Include ALL call options (no ITM probability filter)
- DO NOT exclude any weeks (include all Fridays including third Fridays)
"""
...
print("(Monday entries, days_to_expiry=4, ALL call options, include all weeks)")
print("=" * 80)
...
DB_PATH = Path(__file__).parent / "options_temp_2018_monday.duckdb"
...
df['day_of_week'] = df['date_only_dt'].dt.dayofweek  # Monday=0, Friday=4

# First, check what days_to_expiry values we have for Monday entries
print(f"   Checking available days_to_expiry for Monday entries...")
monday_df = df[df['day_of_week'] == 0].copy()
if len(monday_df) > 0:
    dte_counts = monday_df['days_to_expiry'].value_counts().sort_index()
    print(f"   Days to expiry distribution for Mondays:")
...
# Monday entry, Friday expiration = 4 days (Mon=0, Tue=1, Wed=2, Thu=3, Fri=4)
...
print(f"   ✅ Filtered to {len(weekly_entry):,} Monday entries (days_to_expiry=4)")

if len(weekly_entry) == 0:
    raise RuntimeError("❌ No Monday entries found!")
...
unique_trading_dates = sorted(weekly_entry['date_only'].unique())
...
output_file = output_dir / f"{TEST_YEAR}_options_pessimistic_monday.csv"
