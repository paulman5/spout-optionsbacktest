#!/usr/bin/env python3
"""
Fetch META (Facebook) weekly options with holiday adjustments for holidays folder (2016-2021):
...
- Filter to META ticker only (not GOOGL)
"""
...
def parse_option_ticker(ticker: str, entry_date: date = None):
    """
    Parse option ticker format: O:FB180126C00197500 or O:META180126C00197500
...
print("=" * 80)
print("FETCHING META (2016-2021) FOR HOLIDAYS FOLDER")
...
try:
    DB_PATH = Path(__file__).parent / "options_temp_meta_2016_2021.duckdb"
...
try:
    ...
    FROM glob('s3://{S3_BUCKET}/us_options_opra/day_aggs_v1/2021/01/*.csv.gz')
    LIMIT 1;
    """).fetchone()
...
for TEST_YEAR in YEARS:
    print("\n" + "=" * 80)
    print(f"PROCESSING META {TEST_YEAR}")
    print("=" * 80)
    ...
    # Load data from S3 - filter to FB or META initially, then filter to META only after parsing
    table_name = f"options_{TEST_YEAR}_META_holidays"
    ...
    if not table_exists:
        ...
        # Filter to FB or META initially (we'll filter to META only after parsing)
        ticker_condition = f"(ticker LIKE 'O:FB%' OR ticker LIKE 'O:META%')"
        where_clause = f"WHERE ({ticker_condition})"
        
        print(f"   ⏳ Reading and aggregating files (filtering for FB/META)...")
    ...
    # CRITICAL: Filter to META only (exclude FB - Facebook was renamed to Meta)
    print(f"   Filtering to META only (excluding FB)...")
    before_filter = len(df)
    df = df[df['underlying_symbol'] == 'META'].copy()
    after_filter = len(df)
    print(f"   ✅ Filtered from {before_filter:,} to {after_filter:,} rows (META only)")
    
    if len(df) == 0:
        print(f"   ⚠️  No META data for {TEST_YEAR}, skipping...")
        continue
    ...
    if not stock_file.exists():
        ...
        ticker_data_final['otm_pct'] = ((ticker_data_final['strike'] - ticker_data_final['underlying_spot']) / ticker_data_final['underlying_spot']) * 100
        ticker_data_final['otm_pct'] = ticker_data_final['otm_pct'].round(2)
    ...
    print(f"   All rows are META (not FB): {all(ticker_data_final['underlying_symbol'] == 'META')}")
