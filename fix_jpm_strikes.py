"""
Fix JPM strikes in all holidays CSV files.
For JPM, strikes should be: ticker_strike / 1000 (not / 1000 / 4)
Example: JPM180105C00085000 -> 00085000 / 1000 = 85.0
"""
...
def extract_strike_from_ticker(ticker: str) -> float:
    """
    Extract strike from JPM option ticker and divide by 1000.
    Example: JPM180105C00085000 -> 00085000 / 1000 = 85.0
    """
    try:
        ...
        # Extract strike part: JPM180105C00085000 -> 00085000
        match = re.match(r'([A-Z]+)(\d{6})([CP])(\d+)', ticker)
        if not match:
            return None
        
        strike_str = match.group(4)  # e.g., "00085000"
        strike_raw = float(strike_str)  # e.g., 85000.0
        strike = strike_raw / 1000.0  # e.g., 85.0
...
def main():
    """Main function to fix all JPM holidays files."""
    base_path = Path(__file__).parent / "python-boilerplate" / "data" / "JPM"
