#!/usr/bin/env python3
"""
Calculate UBER monthly options with 4-7% probability ITM for 2016-2025:
...
print("=" * 120)
print(f"CALCULATING UBER MONTHLY OPTIONS (3-7% PROBABILITY ITM) FOR 2016-2025")
print("=" * 120)
print()
...
print("Processing files with 3-7% probability ITM range...")
for year in range(2016, 2026):
    ...
    try:
        ...
        # Filter by 3-7% probability ITM
        filtered = df[(df['probability_itm'] >= 0.03) & 
                      (df['probability_itm'] <= 0.07) & 
                      (df['probability_itm'].notna())]
        
        if len(filtered) == 0:
            print(f"  {year}: No options in 3-7% range")
            continue
        
        print(f"  {year}: Found {len(filtered)} options in 3-7% range")
...
# Create output
output_lines = []
output_lines.append("  UBER Monthly Options (3-7% Probability ITM):")
...
# Insert or replace in the main output file right after UBER weekly section
output_file = Path("itm_liquidation_and_apy_per_year.txt")
with open(output_file, 'r') as f:
    content = f.read()

# Find the insertion point (after UBER weekly TOTAL line)
insertion_marker = "  TOTAL               407         303        43          14.19%      0.1143%\n"

# Check if UBER monthly section already exists and replace it
if "  UBER Monthly Options (3-7% Probability ITM):" in content:
    # Find and replace the existing section
    import re
    pattern = r"  UBER Monthly Options \(3-7% Probability ITM\):.*?TOTAL.*?\n"
    replacement = "\n".join(output_lines) + "\n"
    content = re.sub(pattern, replacement, content, flags=re.DOTALL)
    print("Replaced existing UBER monthly section")
elif "  UBER Monthly Options (4-7% Probability ITM):" in content:
    # Replace old 4-7% section with new 3-7% section
    import re
    pattern = r"  UBER Monthly Options \(4-7% Probability ITM\):.*?TOTAL.*?\n"
    replacement = "\n".join(output_lines) + "\n"
    content = re.sub(pattern, replacement, content, flags=re.DOTALL)
    print("Replaced existing UBER monthly 4-7% section with 3-7%")
    ...
    print("Inserted new UBER monthly section")
else:
...
with open(output_file, 'w') as f:
    f.write(content)
print()
print(f"✅ UBER monthly (3-7% prob ITM) data calculated and updated in: {output_file}")
