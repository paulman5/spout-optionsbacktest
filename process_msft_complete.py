...
def process_msft_complete():
    """Complete MSFT data processing: download, remove PUTs, apply stock splits, merge with historical data"""
    
    print("🚀 Processing MSFT data with complete pipeline...")
    
    # Load MSFT historical data
    stock_file = Path("/Users/paulvanmierlo/spout-optionsbacktest/python-boilerplate/data/MSFT/HistoricalData_MSFT.csv")
    print("📈 Loading MSFT historical stock data...")
    ...
    # Download and process all years (2016-2025)
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                for period in ['monthly', 'weekly']:
                    ...
                    target_file = Path(f"/Users/paulvanmierlo/spout-optionsbacktest/python-boilerplate/data/MSFT/{period}/{year}_options_pessimistic.csv")
                    ...
                    if parquet_file.exists():
                        ...
                        # Apply MSFT stock split adjustments
                        def adjust_strike_msft(row):
                            """Apply MSFT stock split adjustments based on date"""
                            ...
                            # - 2-for-1 on June 12, 1987
                            # - 2-for-1 on June 15, 1992
                            # - 2-for-1 on February 18, 1998
                            # - 2-for-1 on February 18, 2003
                            # - 2-for-1 on September 20, 2003
                            # - 2-for-1 on January 31, 2020
                            
                            if date < pd.to_datetime('2020-01-31'):
                                # Pre-2020-01-31: divide by 2 (for 2-for-1 split on 2020-01-31)
                                adjusted_strike = strike_raw / 2.0
                            else:
                                # On or after 2020-01-31: no adjustment needed
                                adjusted_strike = strike_raw / 1.0
                            
                            return round(adjusted_strike, 2)
                        
                        # Apply strike adjustments
                        df_calls_only['strike'] = df_calls_only.apply(adjust_strike_msft, axis=1)
                        ...
                        if year == 2019:
                            sample_2019 = df_calls_only[df_calls_only['date_only'].dt.year == 2019].iloc[0] if len(df_calls_only[df_calls_only['date_only'].dt.year == 2019]) > 0 else None
                            if sample_2019 is not None:
                                print(f"   🔍 2019 example (pre-2020 split):")
                                print(f"      Strike: ${sample_2019['strike']:.2f} (divided by 2)")
                        elif year == 2021:
    ...
    print(f"\n✅ MSFT data processing completed!")
    
    # Show stock split adjustment summary
    print(f"\n📊 MSFT Stock Split Adjustment Summary:")
    print(f"   Pre-2020-01-31: Strikes divided by 2 (for 2-for-1 split on 2020-01-31)")
    print(f"   On or after 2020-01-31: No adjustment needed")
    
    # Final verification
    print(f"\n🔍 Final verification:")
    monthly_dir = Path("/Users/paulvanmierlo/spout-optionsbacktest/python-boilerplate/data/MSFT/monthly")
    weekly_dir = Path("/Users/paulvanmierlo/spout-optionsbacktest/python-boilerplate/data/MSFT/weekly")
