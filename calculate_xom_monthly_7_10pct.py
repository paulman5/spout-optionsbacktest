#!/usr/bin/env python3
"""
Calculate XOM monthly options with 7-10% probability ITM for 2016-2025:
...
print("=" * 120)
print(f"CALCULATING XOM MONTHLY OPTIONS (7-10% PROBABILITY ITM) FOR 2016-2025")
print("=" * 120)
print()
...
print("Processing files with 7-10% probability ITM range...")
for year in range(2016, 2026):
    ...
    try:
        ...
        # Filter by 7-10% probability ITM
        filtered = df[(df['probability_itm'] >= 0.07) & 
                      (df['probability_itm'] <= 0.10) & 
                      (df['probability_itm'].notna())]
        
        if len(filtered) == 0:
            print(f"  {year}: No options in 7-10% range")
            continue
        
        print(f"  {year}: Found {len(filtered)} options in 7-10% range")
...
# Create output
output_lines = []
output_lines.append("  XOM Monthly Options (7-10% Probability ITM):")
...
# Insert or replace in the main output file right after XOM 10-13% section
output_file = Path("itm_liquidation_and_apy_per_year.txt")
with open(output_file, 'r') as f:
    content = f.read()

# Find the insertion point (after XOM 10-13% TOTAL line)
insertion_marker = "  TOTAL               340         112        20          16.67%      0.3947%\n"

# Check if XOM monthly 7-10% section already exists and replace it
if "  XOM Monthly Options (7-10% Probability ITM):" in content:
    # Find and replace the existing section
    import re
    pattern = r"  XOM Monthly Options \(7-10% Probability ITM\):.*?TOTAL.*?\n"
    replacement = "\n".join(output_lines) + "\n"
    content = re.sub(pattern, replacement, content, flags=re.DOTALL)
    print("Replaced existing XOM monthly 7-10% section")
elif insertion_marker in content:
    # Insert the monthly data right after the 10-13% section
    insertion_text = "\n" + "\n".join(output_lines) + "\n"
    content = content.replace(insertion_marker, insertion_marker + insertion_text)
    print("Inserted new XOM monthly 7-10% section")
else:
...
with open(output_file, 'w') as f:
    f.write(content)
print()
print(f"✅ XOM monthly (7-10% prob ITM) data calculated and updated in: {output_file}")
