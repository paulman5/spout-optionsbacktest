...
def fix_aapl_stock_splits():
    ...
    for year in years_to_process:
        print(f"\n📅 Processing {year} data with stock split adjustments...")
        ...
        for period in ['monthly', 'weekly']:
            ...
            try:
                ...
                # Apply stock split adjustments to strikes
                def adjust_strike_aapl(row):
                    """Apply AAPL stock split adjustments based on date"""
                    ...
                    if date < pd.to_datetime('2014-06-09'):
                        # Pre-2014-06-09: divide by 7 (for 7-for-1 split in 2014)
                        adjusted_strike = strike_raw / 7.0
                    elif date < pd.to_datetime('2020-08-31'):
                        # 2014-06-09 to 2020-08-31: divide by 1 (no adjustment)
                        adjusted_strike = strike_raw / 1.0
                    else:
                        # Post-2020-08-31: divide by 4 (for 4-for-1 split in 2020)
                        adjusted_strike = strike_raw / 4.0
                    
                    return round(adjusted_strike, 2)
                
                # Apply strike adjustments
                df['strike'] = df.apply(adjust_strike_aapl, axis=1)
                ...
                if year == 2016:
                    sample_2016 = df[df['date_only'].dt.year == 2016].iloc[0]
                    print(f"   🔍 2016 example (pre-2014 split):")
                    print(f"      Strike: ${sample_2016['strike']:.2f} (divided by 7)")
                elif year == 2017:
                    sample_2017 = df[df['date_only'].dt.year == 2017].iloc[0]
                    print(f"   🔍 2017 example (post-2014, pre-2020):")
                    print(f"      Strike: ${sample_2017['strike']:.2f} (no adjustment)")
                elif year == 2021:
                    sample_2021 = df[df['date_only'].dt.year == 2021].iloc[0]
                    print(f"   🔍 2021 example (post-2020 split):")
                    print(f"      Strike: ${sample_2021['strike']:.2f} (divided by 4)")
    ...
    # Show split adjustment summary
    print(f"\n📊 Stock Split Adjustment Summary:")
    print(f"   Pre-2014-06-09: Strikes divided by 7 (for 7-for-1 split)")
    print(f"   2014-06-09 to 2020-08-31: No adjustment needed")
    print(f"   Post-2020-08-31: Strikes divided by 4 (for 4-for-1 split)")


if __name__ == "__main__":
    fix_aapl_stock_splits()
