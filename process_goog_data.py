#!/usr/bin/env python3
"""
Process GOOG data to match exact column structure with all historical columns.
"""
...
def extract_strike_from_ticker(ticker):
    ...
    Example: O:GOOG200221C00980000 -> 98000000
    """
    # Remove prefix (O:GOOG) and option type (C/P)
    # The format is: O:GOOG + YYMMDD + C/P + 8-digit strike
    match = re.match(r'O:GOOG\d{6}[CP](\d{8})', ticker)
    if match:
        return int(match.group(1))
    return None

def process_goog_data(file_path):
    """Process GOOG data to match exact column structure."""
    ...
    if 'underlying_spot' in df.columns:
        df['ITM'] = df['strike'].le(df['underlying_spot']).map({True: 'YES', False: 'NO'})
    ...
    # Add missing historical columns (use underlying_spot as base)
    if 'underlying_spot' in df.columns:
        df['underlying_open'] = df['underlying_spot']
        df['underlying_close'] = df['underlying_spot']
        df['underlying_high'] = df['underlying_spot']
        df['underlying_low'] = df['underlying_spot']
...
def main():
    """Process GOOG data for all years."""
    goog_dir = "/Users/paulvanmierlo/spout-optionsbacktest/python-boilerplate/data/GOOG"
    ...
    for year in years:
        ...
        files_to_process = [
            os.path.join(goog_dir, "monthly", f"{year}_options_pessimistic.csv"),
            os.path.join(goog_dir, "weekly", f"{year}_options_pessimistic.csv")
        ]
        
        for file_path in files_to_process:
            if os.path.exists(file_path):
                process_goog_data(file_path)
            else:
                print(f"⚠️ File not found: {file_path}")
    
    print("\n✅ GOOG data processed for all years!")
