#!/usr/bin/env python3
"""
Fix XLK split application - correctly apply 2-for-1 split only before December 5, 2025 and preserve original strikes after
"""

import pandas as pd
from pathlib import Path

def fix_xlk_split_application_final(file_path, split_date, split_ratio):
    """Fix split application - only apply to data before split date, preserve original after"""
    ...
    # For data after split date, ensure original strikes are preserved (not divided)
    if len(after_split) > 0:
        print(f"   Preserving original strike prices for {len(after_split)} rows")
        
        # The current strikes might be incorrectly divided, so we need to restore them
        # Estimate original strikes by multiplying by split ratio
        estimated_original = after_split['strike'] * split_ratio
        
        # Round to 2 decimal places
        after_split['strike'] = estimated_original.round(2)
...
def main():
    """Main function to fix XLK split application"""
    print("🔄 Fixing XLK split application - preserve original strikes after December 5, 2025...")
    ...
    for year in years:
        ...
        for freq in frequencies:
            ...
            if file_path.exists():
                rows_processed = fix_xlk_split_application_final(file_path, split_date, split_ratio)
                total_rows_processed += rows_processed
            else:
                print(f"   File not found: {file_path}")
    
    print(f"\n✅ XLK split application fixed!")
