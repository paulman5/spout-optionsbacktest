#!/usr/bin/env python3
"""
Fetch META weekly options with holiday adjustments for 2016-2021:
...
- Strike price: divide by 1000 only (no /4 division)
"""
...
def parse_option_ticker(ticker: str, entry_date: date = None):
    """
    Parse option ticker format: O:META180126C00197500
    Returns: symbol, expiration_date, option_type, strike
    
    Strike calculation for META:
    - Always: strike = raw_strike / 1000 (no /4 division)
    """
    try:
        ...
        # Parse strike: META always divides by 1000 only
...
print("=" * 80)
print(f"FETCHING META HOLIDAYS DATA FOR {YEARS[0]}-{YEARS[-1]}")
...
try:
    DB_PATH = Path(__file__).parent / "options_temp_meta_holidays.duckdb"
...
for TEST_YEAR in YEARS:
    ...
    # META prices don't need division by 4 - they're already in the correct format
    # (SPY needed /4 because of the strike calculation, but META doesn't)
