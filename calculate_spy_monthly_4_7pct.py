#!/usr/bin/env python3
"""
Calculate SPY monthly options with 4-7% probability ITM for 2016-2025:
...
print("=" * 120)
print(f"CALCULATING SPY MONTHLY OPTIONS (4-7% PROBABILITY ITM) FOR 2016-2025")
print("=" * 120)
print()
...
# Create output
output_lines = []
output_lines.append("  SPY Monthly Options (4-7% Probability ITM):")
...
# Insert or replace in the main output file right after SPY 10-13% section
output_file = Path("itm_liquidation_and_apy_per_year.txt")
with open(output_file, 'r') as f:
    content = f.read()

# Find the insertion point (after SPY 10-13% TOTAL line)
insertion_marker = "  TOTAL             1,032         120        30          25.00%      0.2199%\n"

# Check if SPY monthly 4-7% section already exists and replace it
if "  SPY Monthly Options (4-7% Probability ITM):" in content:
    # Find and replace the existing section
    import re
    pattern = r"  SPY Monthly Options \(4-7% Probability ITM\):.*?TOTAL.*?\n"
    replacement = "\n".join(output_lines) + "\n"
    content = re.sub(pattern, replacement, content, flags=re.DOTALL)
    print("Replaced existing SPY monthly 4-7% section")
    ...
    print("Inserted new SPY monthly 4-7% section")
else:
...
with open(output_file, 'w') as f:
    f.write(content)
print()
print(f"✅ SPY monthly (4-7% prob ITM) data calculated and updated in: {output_file}")
