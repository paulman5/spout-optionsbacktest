#!/usr/bin/env python3
"""
Fix AMZN 2022 data - only divide strike prices by 20, keep premiums unchanged
"""

import pandas as pd
from pathlib import Path

def fix_amzn_2022_strikes(file_path, split_date, split_ratio):
    """Fix 2022 data - only divide strike prices, keep premiums unchanged"""
    ...
    if len(before_split) > 0:
        ...
        # Recalculate premium yields (premiums stay the same, only denominator changes)
        ...
        print(f"   Divided strikes by {split_ratio} for {len(before_split)} rows")
        print(f"   Premiums, open_price, close_price kept unchanged")
...
def main():
    ...
    print("🔄 Fixing AMZN 2022 data - only divide strikes by 20, keep premiums unchanged...")
    ...
    for year in years:
        ...
        for freq in frequencies:
            ...
            if file_path.exists():
                rows_processed = fix_amzn_2022_strikes(file_path, split_date, split_ratio)
