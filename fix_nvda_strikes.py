"""
Fix NVDA strikes in all holidays CSV files.
For NVDA:
- Before July 20, 2021: strike = ticker_strike / 1000 / 40
- On/After July 20, 2021: strike = ticker_strike / 1000
Example: NVDA160108C00031000 -> 31000 / 1000 = 31, then 31 / 40 = 0.775 (before split date)
"""

import pandas as pd
import re
from pathlib import Path
from datetime import datetime


SPLIT_DATE_1 = datetime(2021, 7, 20)  # First split date
SPLIT_DATE_2 = datetime(2024, 6, 10)  # Second split date


def extract_strike_from_ticker(ticker: str, entry_date: datetime) -> float:
    """
    Extract strike from NVDA option ticker.
    Before July 20, 2021: divide by 1000, then by 40
    On/After July 20, 2021 AND Before June 10, 2024: divide by 1000, then by 10
    On/After June 10, 2024: divide by 1000 only
    """
    try:
        # Remove 'O:' prefix if present
        if ticker.startswith('O:'):
            ticker = ticker[2:]
        
        # Extract strike part: NVDA160108C00031000 -> 00031000
        match = re.match(r'([A-Z]+)(\d{6})([CP])(\d+)', ticker)
        if not match:
            return None
        
        strike_str = match.group(4)  # e.g., "00031000"
        strike_raw = float(strike_str) / 1000.0  # e.g., 31.0
        
        # Check date ranges
        if entry_date < SPLIT_DATE_1:
            # Before July 20, 2021: divide by 40
            strike = strike_raw / 40.0  # e.g., 31.0 / 40 = 0.775
        elif entry_date < SPLIT_DATE_2:
            # On/After July 20, 2021 AND Before June 10, 2024: divide by 10
            strike = strike_raw / 10.0  # e.g., 325.0 / 10 = 32.5
        else:
            # On/After June 10, 2024: just divide by 1000
            strike = strike_raw  # e.g., 100.0
        
        return strike
    except (ValueError, IndexError, AttributeError, TypeError):
        return None


def fix_holidays_file(holidays_file: Path):
    """
    Fix strikes in a single holidays CSV file.
    
    Args:
        holidays_file: Path to the holidays CSV file
    """
    print(f"\n{'='*80}")
    print(f"Processing: {holidays_file.name}")
    print(f"{'='*80}")
    
    # Load holidays data
    print(f"📂 Loading holidays data...")
    df = pd.read_csv(holidays_file)
    print(f"   Loaded {len(df):,} rows")
    
    # Check current strikes
    if 'strike' not in df.columns:
        print(f"   ❌ No 'strike' column found!")
        return False
    
    if 'ticker' not in df.columns:
        print(f"   ❌ No 'ticker' column found!")
        return False
    
    if 'date_only' not in df.columns:
        print(f"   ❌ No 'date_only' column found!")
        return False
    
    # Convert date_only to datetime
    df['date_only_dt'] = pd.to_datetime(df['date_only'])
    
    # Show sample of current strikes
    print(f"\n   Sample of current strikes (first 5 rows):")
    for idx, row in df.head(5).iterrows():
        print(f"      {row['ticker']} (date: {row['date_only']}) -> strike: {row['strike']}")
    
    # Extract correct strikes from tickers
    print(f"\n🔄 Extracting correct strikes from tickers...")
    df['strike_corrected'] = df.apply(
        lambda row: extract_strike_from_ticker(row['ticker'], row['date_only_dt']),
        axis=1
    )
    
    # Check how many were successfully parsed
    parsed_count = df['strike_corrected'].notna().sum()
    print(f"   Successfully parsed: {parsed_count:,} / {len(df):,} ({100*parsed_count/len(df):.1f}%)")
    
    if parsed_count < len(df):
        failed = df[df['strike_corrected'].isna()]
        print(f"   ⚠️  Failed to parse {len(failed)} tickers")
        if len(failed) <= 10:
            for idx, row in failed.iterrows():
                print(f"      - {row['ticker']}")
        else:
            for idx, row in failed.head(10).iterrows():
                print(f"      - {row['ticker']}")
            print(f"      ... and {len(failed) - 10} more")
    
    # Check split date distribution
    before_split1 = df['date_only_dt'] < SPLIT_DATE_1
    between_splits = (df['date_only_dt'] >= SPLIT_DATE_1) & (df['date_only_dt'] < SPLIT_DATE_2)
    after_split2 = df['date_only_dt'] >= SPLIT_DATE_2
    print(f"\n   Date distribution:")
    print(f"      Before July 20, 2021: {before_split1.sum():,} rows (divide by 40)")
    print(f"      July 20, 2021 - June 10, 2024: {between_splits.sum():,} rows (divide by 10)")
    print(f"      On/After June 10, 2024: {after_split2.sum():,} rows (divide by 1000 only)")
    
    # Compare old vs new strikes
    df_comparison = df[df['strike_corrected'].notna()].copy()
    if len(df_comparison) > 0:
        df_comparison['strike_diff'] = df_comparison['strike_corrected'] - df_comparison['strike']
        df_comparison['strike_ratio'] = df_comparison['strike_corrected'] / df_comparison['strike']
        
        print(f"\n   Strike comparison (where both exist):")
        print(f"      Average old strike: {df_comparison['strike'].mean():.2f}")
        print(f"      Average new strike: {df_comparison['strike_corrected'].mean():.2f}")
        print(f"      Average difference: {df_comparison['strike_diff'].mean():.2f}")
        print(f"      Average ratio: {df_comparison['strike_ratio'].mean():.4f}")
        
        # Show sample of corrections
        print(f"\n   Sample of corrections (first 5 rows with changes):")
        sample = df_comparison.head(5)
        for idx, row in sample.iterrows():
            print(f"      {row['ticker']} (date: {row['date_only']})")
            print(f"         Old: {row['strike']:.2f} -> New: {row['strike_corrected']:.2f} (diff: {row['strike_diff']:.2f})")
    
    # Update strikes
    print(f"\n💾 Updating strikes...")
    df['strike'] = df['strike_corrected']
    df = df.drop(columns=['strike_corrected', 'date_only_dt'], errors='ignore')
    
    # Save updated file
    print(f"💾 Saving updated file...")
    df.to_csv(holidays_file, index=False)
    print(f"   ✅ Saved {holidays_file}")
    
    return True


def main():
    """Main function to fix all NVDA holidays files."""
    base_path = Path(__file__).parent / "python-boilerplate" / "data" / "NVDA"
    holidays_dir = base_path / "holidays"
    
    if not holidays_dir.exists():
        print(f"❌ Holidays directory not found: {holidays_dir}")
        return
    
    # Get all holidays CSV files
    holidays_files = sorted(holidays_dir.glob("*_options_pessimistic.csv"))
    
    if not holidays_files:
        print(f"❌ No holidays CSV files found in {holidays_dir}")
        return
    
    print(f"Found {len(holidays_files)} holidays files to process")
    print(f"Split dates:")
    print(f"  - Before July 20, 2021: strike = ticker_strike / 1000 / 40")
    print(f"  - July 20, 2021 - June 10, 2024: strike = ticker_strike / 1000 / 10")
    print(f"  - On/After June 10, 2024: strike = ticker_strike / 1000")
    
    # Process each file
    success_count = 0
    for holidays_file in holidays_files:
        if fix_holidays_file(holidays_file):
            success_count += 1
    
    print(f"\n{'='*80}")
    print(f"✅ Processing complete!")
    print(f"   Successfully updated: {success_count} / {len(holidays_files)} files")
    print(f"{'='*80}")


if __name__ == "__main__":
    main()

