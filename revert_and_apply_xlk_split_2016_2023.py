#!/usr/bin/env python3
"""
Fix XLK 2016-2023 data - revert to original strikes and apply 2-for-1 split only before December 5, 2025
"""

import pandas as pd
from pathlib import Path

def revert_and_apply_split_2016_2023(file_path, split_date, split_ratio):
    """Revert to original strikes and apply split only before specified date"""
    print(f"Reverting and fixing {file_path}...")
    ...
    # Extract correct strike prices from ticker symbols (revert to original)
    import re
    ...
    if len(before_split) > 0:
        # Apply split to strike prices ONLY before split date
...
def main():
    """Main function to revert and apply XLK split for 2016-2023"""
    print("🔄 Reverting XLK 2016-2023 data and applying 2-for-1 split only before December 5, 2025...")
    
    split_date = pd.to_datetime('2025-12-05')
    split_ratio = 2
    
    base_dir = Path("/Users/paulvanmierlo/spout-optionsbacktest/python-boilerplate/data/XLK")
    years = [2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023]  # Fix 2016-2023 data
    frequencies = ['weekly', 'monthly']  # Fix weekly and monthly data
    ...
    for year in years:
        ...
        for freq in frequencies:
            ...
            if file_path.exists():
                rows_processed = revert_and_apply_split_2016_2023(file_path, split_date, split_ratio)
                total_rows_processed += rows_processed
            else:
                print(f"   File not found: {file_path}")
    
    print(f"\n✅ XLK 2016-2023 data reverted and split applied!")
