#!/usr/bin/env python3
"""
Calculate UNH monthly options with 7-10% probability ITM for 2016-2025:
...
print("=" * 120)
print(f"CALCULATING UNH MONTHLY OPTIONS (7-10% PROBABILITY ITM) FOR 2016-2025")
print("=" * 120)
print()
...
# Create output
output_lines = []
output_lines.append("  UNH Monthly Options (7-10% Probability ITM):")
...
# Insert or replace in the main output file right after UNH 10-13% section
output_file = Path("itm_liquidation_and_apy_per_year.txt")
with open(output_file, 'r') as f:
    content = f.read()

# Find the insertion point (after UNH 10-13% TOTAL line)
insertion_marker = "  TOTAL               340         115        24          20.00%      0.4131%\n"

# Check if UNH monthly 7-10% section already exists and replace it
if "  UNH Monthly Options (7-10% Probability ITM):" in content:
    # Find and replace the existing section
    import re
    pattern = r"  UNH Monthly Options \(7-10% Probability ITM\):.*?TOTAL.*?\n"
    replacement = "\n".join(output_lines) + "\n"
    content = re.sub(pattern, replacement, content, flags=re.DOTALL)
    print("Replaced existing UNH monthly 7-10% section")
    ...
    print("Inserted new UNH monthly 7-10% section")
else:
...
with open(output_file, 'w') as f:
    f.write(content)
print()
print(f"✅ UNH monthly (7-10% prob ITM) data calculated and updated in: {output_file}")
