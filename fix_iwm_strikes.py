"""
Fix IWM strikes in all holidays CSV files.
For IWM, strikes should be: ticker_strike / 1000 (not / 1000 / 4)
Example: IWM200110C00168000 -> 168000 / 1000 = 168.0
"""
...
def extract_strike_from_ticker(ticker: str) -> float:
    """
    Extract strike from IWM option ticker and divide by 1000.
    Example: IWM200110C00168000 -> 168000 / 1000 = 168.0
    """
    try:
        ...
        # Extract strike part: IWM200110C00168000 -> 00168000
        match = re.match(r'([A-Z]+)(\d{6})([CP])(\d+)', ticker)
        if not match:
            return None
        
        strike_str = match.group(4)  # e.g., "00168000"
        strike_raw = float(strike_str)  # e.g., 168000.0
        strike = strike_raw / 1000.0  # e.g., 168.0
