#!/usr/bin/env python3
"""
Calculate WMT monthly options with 4-7% probability ITM for 2016-2025:
...
print("=" * 120)
print(f"CALCULATING WMT MONTHLY OPTIONS (4-7% PROBABILITY ITM) FOR 2016-2025")
print("=" * 120)
print()
...
# Create output
output_lines = []
output_lines.append("  WMT Monthly Options (4-7% Probability ITM):")
...
# Insert or replace in the main output file right after WMT weekly section
output_file = Path("itm_liquidation_and_apy_per_year.txt")
with open(output_file, 'r') as f:
    content = f.read()

# Find the insertion point (after WMT weekly TOTAL line)
insertion_marker = "  TOTAL               529         389        39          10.03%      0.0684%\n"

# Check if WMT monthly section already exists and replace it
if "  WMT Monthly Options (4-7% Probability ITM):" in content:
    # Find and replace the existing section
    import re
    pattern = r"  WMT Monthly Options \(4-7% Probability ITM\):.*?TOTAL.*?\n"
    replacement = "\n".join(output_lines) + "\n"
    content = re.sub(pattern, replacement, content, flags=re.DOTALL)
    print("Replaced existing WMT monthly section")
    ...
    print("Inserted new WMT monthly section")
else:
...
with open(output_file, 'w') as f:
    f.write(content)
print()
print(f"✅ WMT monthly (4-7% prob ITM) data calculated and updated in: {output_file}")
