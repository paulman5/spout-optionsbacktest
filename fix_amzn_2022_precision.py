#!/usr/bin/env python3
"""
Fix AMZN 2022 data - multiply open, close, low prices by 20 with 2 decimal precision
"""
...
def fix_amzn_2022_prices(file_path):
    """Multiply open, close, low prices by 20 for all 2022 data with 2 decimal precision"""
    ...
    # Multiply open, close, low prices by 20 for ALL rows with 2 decimal precision
    df['open_price'] = (df['open_price'] * 20).round(2)
    df['close_price'] = (df['close_price'] * 20).round(2)
    df['low_price'] = (df['low_price'] * 20).round(2)
    df['high_price'] = (df['high_price'] * 20).round(2)
    ...
    print(f"   Applied 2 decimal precision to all price fields")
...
def main():
    ...
    print("🔄 Fixing AMZN 2022 data - multiply open, close, low prices by 20 with 2 decimal precision...")
